...
dataset_csv = pd.read_csv("dataset1.csv", sep=";",decimal=",")



print(type(dataset_csv))
dataset_csv.to_excel("novo.xlsx", index=False)
...
def getSeriesByName(dataFrame, column):
    return dataFrame.get(column)

readingsOf2022 = getSeriesByName(notas_pisa, "reading_2022") 
...
salarios = pd.Series(["junior", "pleno", "senior"])

def calcularSalarioDoDev(salario,posicao):

    tabelaSalarial = pd.DataFrame([{"junior": 3500, "pleno": 5500, "senior": 9000 }])
    print(type(tabelaSalarial))
    item = tabelaSalarial[posicao].item()
    
    if salario > item:
            print("acima da tabela salarial")
    elif salario == item:
        print("dentro da media salarial")
    else : print("abaixo da tabela salarial")
    
calcularSalarioDoDev(5500, "senior")    
